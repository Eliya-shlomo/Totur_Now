import { anthropicClient, isAnthropicConfigured } from '#config/anthropic.js';
import {
 ...
 * merged against a real signature. **PR 3.3 put the Anthropic call in front of it and
 * kept everything below as the `catch`** — the §8.1 path that runs when the model
 * fails, times out or comes back under the confidence floor. 3.4's diff did not change
 * when this landed; the same function simply started returning real topics.
 *
...
const defaultDeps = {
  ...
  createMessage: (params, options) => anthropicClient.messages.create(params, options),
...
export async function classifyQuestion(input = {}, deps = {}) {
  ...
  try {
    if (!configured) return fallback('ANTHROPIC_API_KEY is not configured');
    if (!promptReady) return fallback('llm.prompt.js still holds its placeholder');

    const topicTree = await loadTaxonomy();
    const { system, messages } = buildMessages({ rawText, imageUrls, declaredLevel, topicTree });
    ...
    const response = await callWithTimeout(
      createMessage,
      {
        ...
        system,
        messages,
        // The §8.1 guardrail, in the form this API actually has. `effort` is
        // deliberately unset — it is not a parameter this model accepts.
 ...
 * student waits for all of them. `config/anthropic.js` sets `maxRetries: 0` for the
 * same reason; the race is what holds if that ever changes underneath us.
 *
...
async function callWithTimeout(createMessage, params, timeoutMs) {
  ...
  try {
    return await Promise.race([
      createMessage(params, { timeout: timeoutMs, signal: controller.signal }),
      deadline,
    ]);
  } finally {
    clearTimeout(timer);
 ...
 * one `catch` above turns any throw here into the fallback.
 */
function readJson(response) {
  const text = response?.content?.find((block) => block?.type === 'text')?.text;

  if (typeof text !== 'string') throw new Error('the response carried no text block');
